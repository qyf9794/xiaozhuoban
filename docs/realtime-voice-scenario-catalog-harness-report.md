...
022. [pass] tools=weather.set_city; command=看看洛杉矶天气
...
037. [pass] tools=note.write; command=把会议纪要追加到便签
038. [pass] tools=note.clear; command=清空便签内容
039. [pass] tools=todo.add_item; command=添加待办买咖啡豆
...
096. [pass] tools=board.add_widget; command=再打开一个电视
097. [pass] tools=board.add_widget; command=再打开一个天气
098. [pass] tools=board.add_widget; command=再打开一个倒计时
099. [pass] tools=board.add_widget; command=再打开一个待办
100. [pass] tools=board.add_widget; command=再打开一个剪贴板
101. [pass] tools=board.add_widget; command=再打开一个翻译
102. [pass] tools=board.add_widget; command=再打开一个计算器
103. [pass] tools=board.add_widget; command=再打开一个行情
104. [pass] tools=board.add_widget; command=再打开一个新闻
105. [pass] tools=board.add_widget; command=再打开一个世界时钟
106. [pass] tools=board.add_widget; command=再打开一个录音机
...
110. [pass] tools=note.write,todo.add_item; command=先记下买票，然后添加待办订酒店
...
122. [pass] tools=tv.select_channel; command=电影频道打开，场景1
...
131. [pass] tools=widget.resize; command=音乐面板放大，场景1
...
142. [pass] tools=tv.select_channel; command=电影频道打开，场景2
...
151. [pass] tools=widget.resize; command=音乐面板放大，场景2
...
171. [pass] tools=widget.resize; command=音乐面板放大，场景3
...
191. [pass] tools=widget.resize; command=音乐面板放大，场景4
...
201. [pass] tools=app.sidebar.set,app.settings.open; command=先把左侧边栏收起，然后打开设置检查语音入口
202. [pass] tools=app.fullscreen.set,app.command_palette.open; command=进入全屏后马上退出，再打开命令面板找音乐播放器
203. [pass] tools=app.sidebar.set,widget.bring_to_front,widget.focus; command=把侧边栏显示回来，同时把设置窗口放到最前面
204. [pass] tools=app.settings.open,app.command_palette.open; command=打开设置，切到语音相关页面，如果没有就打开命令面板
205. [pass] tools=app.sidebar.set,board.auto_align; command=我想专心一下，隐藏侧栏并把当前桌面整理整齐
206. [pass] tools=app.fullscreen.set,app.command_palette.open; command=退出全屏，打开搜索面板，然后输入天气两个字
207. [pass] tools=app.fullscreen.set; command=进入沉浸模式，同时不要关闭正在播放的音乐
...
210. [pass] tools=app.sidebar.set,widget.bring_to_front,widget.focus; command=先显示侧边栏，再把音乐和天气两个窗口都放到前面
211. [pass] tools=app.settings.open,music.auth_status; command=打开设置后帮我检查有没有登录音乐的入口
212. [pass] tools=app.fullscreen.set,widget.focus; command=我刚才误触全屏了，恢复普通窗口并聚焦便签
213. [pass] tools=app.sidebar.set,app.ai_dialog.open; command=隐藏侧栏，打开 AI 小工具窗口，名字先叫每日摘要
214. [pass] tools=app.command_palette.open; command=把命令面板打开，如果当前在全屏就先退出
215. [pass] tools=app.sidebar.set,tv.fullscreen,tv.play,widget.fullscreen_focus; command=进入全屏看电视，同时把侧边栏藏起来
...
218. [pass] tools=app.command_palette.open; command=打开搜索命令面板并准备查找世界时钟
219. [pass] tools=app.sidebar.set,widget.bring_to_front,widget.focus; command=把侧边栏切换一下，再把表盘时钟放最前
...
227. [pass] tools=board.rename; command=把当前桌板改成语音回归测试，不要删除任何小工具
...
238. [pass] tools=board.create,board.add_widget; command=创建阅读桌板，打开便签、翻译和世界时钟
239. [pass] tools=app.command_palette.open,board.switch; command=切回上一个桌板，如果找不到就打开命令面板
240. [pass] tools=board.auto_align,widget.remove; command=整理桌板之后把留言板关闭，不要发送留言
241. [pass] tools=widget.move,widget.resize; command=把音乐播放器移到左下角，再把封面区域放大一点
242. [pass] tools=widget.resize,widget.move,widget.bring_to_front,widget.focus; command=把天气卡片缩小，电视窗口放到右上角并置顶
243. [pass] tools=widget.remove,board.add_widget; command=关闭留言板，然后打开一个新的便签实例
244. [pass] tools=widget.fullscreen_focus; command=把电视窗口全屏，退出后仍然放在最前面
245. [pass] tools=widget.move; command=把录音机移到音乐旁边，两个窗口都不要遮住
246. [pass] tools=widget.move; command=把世界时钟放到右侧，把表盘时钟放到中间
247. [pass] tools=widget.resize,headline.request_refresh; command=把行情窗口调宽，同时刷新重大新闻
248. [pass] tools=board.add_widget; command=再打开一个倒计时，用完后把旧的倒计时关闭
249. [pass] tools=widget.move,widget.resize; command=把计算器和换算器并排放，宽度都调小
250. [pass] tools=widget.move,widget.focus; command=把翻译窗口拖到便签下面，并聚焦翻译输入框
251. [pass] tools=widget.resize,widget.bring_to_front,widget.focus; command=把待办窗口放大，完成后把便签放到最前
252. [pass] tools=widget.remove; command=关闭天气和新闻，只保留音乐、电视、待办
253. [pass] tools=board.add_widget,widget.move; command=打开剪贴板后把它固定在屏幕右侧
254. [pass] tools=widget.resize; command=把表盘时钟调小一点，别挡住音乐封面
255. [pass] tools=widget.move,tv.fullscreen,widget.fullscreen_focus; command=把电视从右上角移到左侧，再打开全屏预览
256. [pass] tools=widget.remove; command=关闭所有临时小工具，但保留音乐播放器
...
262. [pass] tools=music.play; command=我要听陈奕迅的十年，不要继续上一首
...
274. [pass] tools=music.play; command=播放 Coldplay Yellow，别解析成颜色翻译
...
280. [pass] tools=music.search; command=播放轻松音乐时重新搜索，不要沿用上一首
281. [pass] tools=music.search; command=我想听点轻松的中文歌，先搜索不要立刻播放
282. [pass] tools=music.search; command=放一点适合写代码的纯音乐，结果要重新搜索
283. [pass] tools=music.search; command=来点不吵的背景音乐，别播刚才那首
284. [pass] tools=music.search,widget.move; command=找适合睡前的歌，播放器放在桌面左下
...
289. [pass] tools=music.auth_status; command=不要播放试听版，优先用已登录的音乐账号
290. [pass] tools=music.search; command=给我找运动时听的歌，并把下一首按钮准备好
291. [pass] tools=music.search; command=换成轻松一点的，不要继续现在的歌曲
292. [pass] tools=music.search; command=搜索雨天适合听的音乐，只要歌曲不要电台
293. [pass] tools=tv.pause,music.search; command=找午休背景音乐，播放前把电视暂停
...
299. [pass] tools=music.pause,music.search; command=先暂停当前歌曲，再找轻柔民谣
...
308. [pass] tools=tv.select_channel,tv.play,widget.move; command=打开 CCTV6，同时把电视窗口放到右上角
...
311. [pass] tools=board.add_widget,widget.move; command=打开电视，但不要遮住天气卡片
312. [pass] tools=tv.play,countdown.set; command=播放 CCTV1 综合频道，再设十分钟倒计时
313. [pass] tools=tv.play,tv.select_channel; command=帮我看新闻直播，优先 CCTV13
...
325. [pass] tools=weather.set_city; command=广州天气怎么样，同时刷新空气相关摘要
...
331. [pass] tools=weather.set_city,worldClock.set_zones; command=查东京天气，同时打开东京世界时钟
...
360. [pass] tools=assistant.reply; command=我说打开时钟时优先打开表盘时钟
...
408. [pass] tools=board.add_widget; command=五美元大概是多少人民币，先打开换算器等待我确认汇率
...
427. [pass] tools=widget.move; command=把新闻窗口放到右侧，行情放到左侧
428. [pass] tools=market.set_indices,widget.bring_to_front; command=查询上证指数后把市场窗口置顶
429. [pass] tools=board.switch,headline.request_refresh; command=打开财经观察桌板并刷新重大新闻
...
446. [pass] tools=board.add_widget; command=会议开始，打开录音机、便签和倒计时
447. [pass] tools=recorder.stop,messageBoard.send; command=停止录音并把文件状态写到留言板
448. [pass] tools=recorder.play; command=播放刚才录音，如果没有录音就告诉我
449. [pass] tools=widget.move; command=录音机放到音乐旁边，避免遮住封面
...
453. [pass] tools=recorder.stop,clipboard.add_text; command=停止录音并打开剪贴板保存测试编号
...
460. [pass] tools=recorder.pause,widget.focus; command=录音回放暂停后聚焦待办窗口
461. [pass] tools=widget.remove; command=关闭留言板，不要把关闭两个字发出去
462. [pass] tools=messageBoard.send; command=留言板发送：我在测试多轮语音
463. [pass] tools=widget.remove; command=把留言板收起来，同时保留便签
...
471. [pass] tools=assistant.runtime_diagnostics; command=留言板不要重复发送刚才那句话
...
497. [pass] tools=music.pause,recorder.start,board.add_widget; command=把音乐暂停，开始录音，然后打开倒计时
...
506. [pass] tools=tv.select_channel; command=把电视全屏，等下先别全屏，先切 CCTV5
...
545. [pass] tools=widget.move; command=把音乐播放控件居中，登录按钮别挡封面
...
581. [pass] tools=board.switch,board.add_widget; command=打开学习桌板，启动翻译和便签
...
604. [pass] tools=todo.add_item; command=添加待办订酒店和买高铁票
...
614. [pass] tools=todo.add_item; command=添加待办：打印行程单
615. [pass] tools=calculator.set_display; command=把旅行预算 1999 加 299 算一下
616. [pass] tools=widget.remove,board.add_widget,weather.set_city; command=关闭电视，打开音乐和天气
...
621. [pass] tools=board.add_widget; command=打开音乐播放器，如果工具没加载就先加载音乐模块
622. [pass] tools=app.command_palette.open,widget.focus,weather.set_city; command=我要找天气工具，先打开命令面板再聚焦天气
623. [pass] tools=music.play; command=播放轻松音乐前只加载音乐相关工具，不要全量发送
624. [pass] tools=board.add_widget,dialClock.set_night_mode; command=打开表盘时钟时加载时钟模块，不要加载新闻行情
...
632. [pass] tools=converter.set; command=计算两公斤换算克时加载计算器和换算器
633. [pass] tools=headline.request_refresh; command=刷新新闻时加载新闻模块，不要顺手加载全球指数
634. [pass] tools=market.set_indices; command=看美股三大指数时加载行情模块
635. [pass] tools=recorder.start; command=开始录音时加载录音机控制工具
...
640. [pass] tools=assistant.runtime_diagnostics; command=模块选择失败时降级给完整工具摘要再重试一次
641. [pass] tools=music.play; command=音乐已经登录了，播放王菲红豆不要用试听源
642. [pass] tools=music.auth_status; command=如果音乐 token 不可用，告诉我为什么还是试听
643. [pass] tools=music.auth_status; command=登录按钮还在就先不要自动播放音乐
...
647. [pass] tools=note.write; command=播放完整歌曲失败时把原因写到便签
648. [pass] tools=music.auth_status; command=不要因为已登录界面就假定完整播放成功
649. [pass] tools=widget.move,widget.resize; command=音乐播放器右上角显示登录按钮时不要挡住封面
650. [pass] tools=music.play; command=播放轻松音乐时优先使用已登录账号曲库
651. [pass] tools=assistant.reply; command=如果只能试听，就提示需要开发者 token
652. [pass] tools=music.search; command=重新授权音乐账号后再搜索王菲红豆
653. [pass] tools=music.auth_status; command=登录后刷新音乐播放器状态
...
657. [pass] tools=assistant.runtime_diagnostics; command=搜索结果出现但不能播放时不要一直找小工具
...
665. [pass] tools=assistant.runtime_diagnostics; command=播放音乐失败时记录搜索词、工具名和错误
666. [pass] tools=assistant.runtime_diagnostics; command=关闭留言板成功后记录窗口移除状态
667. [pass] tools=assistant.runtime_diagnostics; command=打开表盘时钟如果重复回复，也记录重复次数
...
674. [pass] tools=assistant.runtime_diagnostics; command=音乐试听模式出现时记录是否已登录
675. [pass] tools=assistant.runtime_diagnostics; command=计时器声音出现时记录来源组件
676. [pass] tools=assistant.runtime_diagnostics; command=打开设置失败时记录路由和当前桌板
677. [pass] tools=assistant.runtime_diagnostics; command=桌面整理确认弹窗出现时记录等待确认
...
680. [pass] tools=assistant.runtime_diagnostics; command=测试结束后导出今天的语音诊断摘要
...
683. [pass] tools=assistant.reply; command=连接后我说在吗，请先回复再等待下一句
684. [pass] tools=board.add_widget; command=如果工具目录没加载完，先用分级策略打开音乐
...
687. [pass] tools=assistant.runtime_diagnostics; command=把所有小桌板窗口能力按需加载给 realtime
688. [pass] tools=assistant.runtime_diagnostics; command=先解析本地高置信命令，低于零点九交给 realtime
689. [pass] tools=music.play; command=播放陈奕迅十年如果缺音乐工具，就加载音乐工具后重试
690. [pass] tools=board.add_widget,dialClock.set_night_mode; command=打开时钟时如果歧义，优先表盘并说明不是世界时钟
691. [pass] tools=assistant.runtime_diagnostics; command=多轮对话里不要忘记刚才已经打开音乐播放器
...
697. [pass] tools=assistant.runtime_diagnostics; command=如果 realtime 回复没有工具，补发该模块工具清单
698. [pass] tools=assistant.runtime_diagnostics; command=第一次发送全局工具摘要，后续只发送选中模块详情
699. [pass] tools=assistant.runtime_diagnostics; command=并发执行天气和新闻时分别记录前端成功状态
