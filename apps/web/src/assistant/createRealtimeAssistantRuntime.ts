...
export function shouldFallbackUnhandledVoiceTranscriptToHarness(input: string): boolean {
  ...
  return /(关闭|关掉|收起|打开|唤出|调出|整理|排列|对齐|全屏|侧栏|侧边栏|工作台|设置|命令面板|倒计时|计时|留言板|音乐|歌曲|歌|听|播放|放一下|放一首|来一首|来个|来点|搜|搜索|查找|找一点|找一下|找歌|找音乐|王菲|陈奕迅|周杰伦|孙燕姿|Beyond|李宗盛|Taylor Swift|Adele|Coldplay|红豆|十年|时钟|表盘|几点|时间|时区|天气|新闻|头条|行情|指数|股票|股价|个股|翻译|换算|壁纸|背景|桌面背景|换壁纸|换背景|小工具|窗口|组件|面板)/i.test(
    normalized
  );
}
...
export interface RealtimeAssistantRuntime {
  ...
  appendTrustedRealtimeMessage: (input: string) => boolean;
  disconnect: () => void;
...
export function createRealtimeAssistantRuntime(options: {
  ...
  const runtimeApi: RealtimeAssistantRuntime = {
    ...
    appendTrustedRealtimeMessage(input) {
      if (!adapter.sendTextCommand || !runtimeController.mode.startsWith("realtime_")) return false;
      try {
        noteRealtimeActivity("trusted_workbench_event");
        adapter.sendTextCommand(input, { commandTraceId: `workbench_event_${Date.now()}` });
        return true;
      } catch {
        return false;
      }
