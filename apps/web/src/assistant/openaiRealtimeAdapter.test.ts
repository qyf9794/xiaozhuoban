...
describe("OpenAI realtime adapter helpers", () => {
  it("detects semantically truncated assistant replies", () => {
    ...
    expect(isRealtimeAssistantReplyComplete("不过")).toBe(false);
  });
  ...
  it("normalizes realtime music keywords arguments into query", () => {
    const calls: unknown[] = [];
    const adapter = new OpenAIRealtimeWebRtcAdapter({
      onFunctionCall(call) {
        calls.push(call);
    ...
    (adapter as unknown as { handleFunctionCall: (call: unknown) => void }).handleFunctionCall({
      id: "call_music_keywords",
      name: "music.search",
      arguments: { widgetId: "wi_music", keywords: ["轻松", "音乐"] },
      source: "realtime"
    });
    ...
    expect(calls).toEqual([
      expect.objectContaining({
        name: "music.search",
        arguments: { widgetId: "wi_music", query: "轻松 音乐" }
      })
    ]);
  });

  it("repairs drifted music entities from the final transcript without rewriting relative result controls", () => {
    const calls: unknown[] = [];
    const adapter = new OpenAIRealtimeWebRtcAdapter({
      onFunctionCall(call) {
        calls.push(call);
    ...
    adapter.updateTools([
      {
        ...
        argumentKeys: ["widgetId", "query", "resultIndex"],
        scope: "widget-detail",
        widgetType: "music",
        requiresTarget: true
      }
    ...
    Object.assign(adapter as unknown as { sessionReady: boolean; activeCommandTraceId: string }, {
      sessionReady: true,
      activeCommandTraceId: "trace_music_entity"
    });
    const transcripts = (adapter as unknown as { realtimeTraceUserTranscripts: Map<string, { input: string }> }).realtimeTraceUserTranscripts;
    transcripts.set("trace_music_entity", { input: "最后听莫文蔚的盛夏的果实" });

    (adapter as unknown as { handleFunctionCall: (call: unknown) => void }).handleFunctionCall({
      id: "call_music_entity",
      name: "music.play",
      arguments: { widgetId: "wi_music", query: "莫文蔚 剩下的果子" },
      source: "realtime"
    });
    ...
    expect(calls).toEqual([
      expect.objectContaining({
        name: "music.play",
        arguments: { widgetId: "wi_music", query: "莫文蔚的盛夏的果实" }
      })
    ]);

    Object.assign(adapter as unknown as { activeCommandTraceId: string }, { activeCommandTraceId: "trace_music_relative" });
    transcripts.set("trace_music_relative", { input: "播放第一首" });
    (adapter as unknown as { handleFunctionCall: (call: unknown) => void }).handleFunctionCall({
      id: "call_music_relative",
      name: "music.play",
      arguments: { widgetId: "wi_music", resultIndex: 0 },
      source: "realtime"
    });
    ...
    Object.assign(adapter as unknown as { activeCommandTraceId: string; activeScopedToolSelection: unknown }, {
      activeCommandTraceId: "trace_music_misheard_relative",
      activeScopedToolSelection: {
        selectedModule: "music",
        targetHint: "搜索结果第一首",
        userCommand: "播放搜索结果里的第一首"
      }
    });
    transcripts.set("trace_music_misheard_relative", { input: "播放中国所接轨里的第一首" });
    (adapter as unknown as { handleFunctionCall: (call: unknown) => void }).handleFunctionCall({
      id: "call_music_misheard_relative",
      name: "music.play",
      arguments: { widgetId: "wi_music", query: "中国所接轨里的第一首" },
      source: "realtime"
    });
  ...
  it("passes every reminder candidate to scoped Realtime without a local semantic rewrite", () => {
    const sent: unknown[] = [];
    ...
    (adapter as unknown as { handleFunctionCall: (call: unknown) => void }).handleFunctionCall({
      ...
      arguments: {
        selectedModule: "todo",
        candidateTools: ["todo.add_item", "countdown.set"],
        userCommand: "三十分钟后提醒我喝水"
      },
      source: "realtime"
    });

    expect(JSON.stringify(sent)).toContain("todo__dot__add_item");
    expect(JSON.stringify(sent)).toContain("countdown__dot__set");
    expect(diagnostics).not.toEqual(expect.arrayContaining([
      expect.objectContaining({ type: "realtime.tool_selection.semantic_boundary_rewrite" })
    ]));
  });
  ...
  it("does not attach music play follow-up when a pure open-player command is miscalled as play", () => {
    ...
    Object.assign(adapter as unknown as { sessionReady: boolean; activeScopedToolSelection: unknown }, {
      sessionReady: true,
      activeScopedToolSelection: { selectedModule: "music", targetHint: "音乐播放器", userCommand: "打开音乐播放器" }
    });

    (adapter as unknown as { handleFunctionCall: (call: unknown) => void }).handleFunctionCall({
      id: "call_open_player_as_play",
      name: "music.play",
      arguments: { query: "打开音乐播放器" },
      source: "realtime"
    });
    ...
    expect((calls[0] as { arguments?: { followUp?: unknown } }).arguments?.followUp).toBeUndefined();
  });
  ...
  it("forces a second-stage tool call after realtime selector results", () => {
    const events = createRealtimeToolResultEvents(
      { id: "call_selector", name: "assistant.select_tool", arguments: {}, source: "realtime" },
      { status: "success", message: "已选择工具" },
      { responseMode: "text", continueResponse: true, toolChoice: "required" }
    );
  ...
  it("can force realtime text commands through the selector function", () => {
    const events = createRealtimeTextCommandEvents("播放 Tinariwen", {
      toolChoice: { type: "function", name: "assistant__dot__select_tool" }
    });
  ...
  it("continues voice realtime after active selector response finishes", () => {
    ...
    expect(sent[1]).toEqual({
      type: "response.create",
      response: { output_modalities: ["audio"], max_output_tokens: 160, instructions: expect.any(String), tool_choice: "required" }
    });
  });
  ...
  it("continues voice realtime after final tool response finishes", () => {
    ...
    expect(sent[1]).toEqual({
      ...
      response: {
        ...
        max_output_tokens: 160,
        instructions: expect.stringContaining("已播放。"),
        tools: [],
        tool_choice: "none"
      }
    });
  ...
  it("normalizes message-board payload aliases before Harness validation", () => {
    const calls: Array<{ id: string; name: string; arguments: Record<string, unknown> }> = [];
    const adapter = new OpenAIRealtimeWebRtcAdapter({
      onFunctionCall(call) {
        calls.push({ id: call.id, name: call.name, arguments: call.arguments as Record<string, unknown> });
    ...
    adapter.updateContext({
      boardId: "board_1",
      boardName: "默认桌板",
      widgetCountsByType: { messageBoard: 1 },
      widgets: [
        { widgetId: "wi_message", definitionId: "wd_message", type: "messageBoard", name: "留言板", order: 1, summary: "empty" }
      ]
    });
    Object.assign(adapter as unknown as { sessionReady: boolean }, { sessionReady: true });

    (adapter as unknown as { handleFunctionCall: (call: unknown) => void }).handleFunctionCall({
      id: "call_message_content_alias",
      name: "messageBoard.send",
      arguments: { content: "语音测试已完成" },
      source: "realtime"
    });
    ...
    expect(calls).toEqual([
      {
        id: "call_message_content_alias",
        name: "messageBoard.send",
        arguments: { widgetId: "wi_message", text: "语音测试已完成" }
      }
    ]);
  });
  ...
  it("keeps interrupted selector calls so a completed command is not swallowed by VAD", async () => {
    ...
    (
      ...
      item: {
        ...
        arguments: JSON.stringify({
          ...
          candidateTools: ["music.play", "music.search", "board.add_widget"],
          confidence: 0.92
        })
      }
    });
    ...
    expect(serializedSent).toContain("session.update");
    expect(serializedSent).toContain("music__dot__play");
    expect(serializedSent).not.toContain("local_add_widget_shortcut");
  });

  it("continues to scoped realtime execution when selector returns only a module hint", () => {
    const diagnostics: Array<{ type: string; status?: string; toolName?: string; errorCode?: string; data?: unknown }> = [];
    const sent: unknown[] = [];
    const adapter = new OpenAIRealtimeWebRtcAdapter({
      onDiagnostic(event) {
        diagnostics.push(event);
    ...
    adapter.updateContext({
      ...
      boardName: "默认桌板",
      widgetCountsByType: { music: 1 },
      widgets: [{ widgetId: "wi_music", definitionId: "wd_music", type: "music", name: "音乐播放器", order: 1, summary: "待命" }]
    });
    Object.assign(adapter as unknown as { sessionReady: boolean; dataChannel: { readyState: string; send: (payload: string) => void } }, {
      sessionReady: true,
      dataChannel: {
    ...
    (adapter as unknown as { activeRealtimeResponseTraceId: string }).activeRealtimeResponseTraceId = "trace_module_only";
    (adapter as unknown as { realtimeTraceUserTranscripts: Map<string, { input: string }> }).realtimeTraceUserTranscripts.set(
      "trace_module_only",
      { input: "播放 Tinariwen 的 Sastanaqqam" }
    );

    (adapter as unknown as { handleFunctionCall: (call: unknown) => void }).handleFunctionCall({
      id: "select_module_only",
      name: "assistant.select_tool",
      arguments: { selectedModule: "music", targetHint: "Tinariwen Sastanaqqam", userCommand: "播放 Tinariwen 的 Sastanaqqam" },
      source: "realtime"
    });
    ...
    expect(diagnostics).toEqual(expect.arrayContaining([
      expect.objectContaining({
        type: "realtime.tool_selection.success",
        status: "success",
        toolName: expect.stringMatching(/^music\./),
        data: expect.objectContaining({ selectedModule: "music" })
      })
    ]));
    expect(diagnostics).not.toEqual(expect.arrayContaining([
      expect.objectContaining({ type: "realtime.tool_selection.failed", errorCode: "TOOL_SELECTION_CONTEXT_MISSING" })
    ]));
    const serializedSent = JSON.stringify(sent);
    expect(serializedSent).toContain("session.update");
    expect(serializedSent).toContain("music__dot__play");
  ...
  it("falls back when a voice transcript finishes without a command tool call", async () => {
    ...
    ).handleRealtimeEventData({ type: "response.created", response: { id: "resp_close_all_followup" } });
    (
      adapter as unknown as {
        handleRealtimeEventData: (event: Record<string, unknown>) => void;
      }
    ).handleRealtimeEventData({ type: "response.done", response: { id: "resp_close_all_followup" } });
    await Promise.resolve();
  ...
  it("does not run transcript fallback after a direct realtime tool call", async () => {
    ...
    ).handleRealtimeEventData({ type: "response.created", response: { id: "resp_music_tool_result_followup" } });
    (
      adapter as unknown as {
        handleRealtimeEventData: (event: Record<string, unknown>) => void;
      }
    ).handleRealtimeEventData({ type: "response.done", response: { id: "resp_music_tool_result_followup" } });
    await Promise.resolve();
  ...
  it("does not run transcript fallback after realtime selects candidate tools", async () => {
    const fallbackInputs: string[] = [];
    const diagnostics: Array<{ type: string; commandTraceId?: string; status?: string; operationId?: string; toolName?: string }> = [];
    const adapter = new OpenAIRealtimeWebRtcAdapter({
      onUnhandledUserTranscript(input) {
    ...
    Object.assign(adapter as unknown as { sessionReady: boolean; dataChannel: { readyState: string; send: (payload: string) => void } }, {
      ...
      dataChannel: {
        readyState: "open",
        send() {
          // Test only needs adapter state transitions.
        }
      }
    });
    ...
    ).handleRealtimeEventData({ type: "input_audio_buffer.speech_started", item_id: "item_voice_selector" });
    (
      adapter as unknown as {
        handleRealtimeEventData: (event: Record<string, unknown>) => void;
      }
      ...
      type: "conversation.item.input_audio_transcription.completed",
      item_id: "item_voice_selector",
      transcript: "找一点北欧爵士，先不要播放"
    });
    (
      adapter as unknown as {
        handleRealtimeEventData: (event: Record<string, unknown>) => void;
      }
    ).handleRealtimeEventData({ type: "response.created", response: { id: "resp_selector" } });
    (
      adapter as unknown as {
        handleRealtimeEventData: (event: Record<string, unknown>) => void;
      }
      ...
      call_id: "call_select_music",
      name: "assistant__dot__select_tool",
      arguments: JSON.stringify({ candidateTools: ["music.search"], selectedModule: "music", targetHint: "北欧爵士", userCommand: "找一点北欧爵士，先不要播放" })
    });
    (
      adapter as unknown as {
        handleRealtimeEventData: (event: Record<string, unknown>) => void;
      }
    ).handleRealtimeEventData({ type: "response.done", response: { id: "resp_selector" } });
    await Promise.resolve();
  ...
  it("corrects realtime tool selections outside the local exposure plan when the command is explicit", () => {
  ...
  it("passes every direct-audio window candidate to scoped Realtime", () => {
    ...
    adapter.updateContext({
      boardId: "board_1",
      boardName: "默认桌板",
      viewport: { width: 1280, height: 720, mode: "desktop", fullscreen: false },
      widgetCountsByType: { weather: 1 },
      widgets: [
        {
          widgetId: "wi_weather",
    ...
    Object.assign(adapter as unknown as { sessionReady: boolean; activeCommandTraceId: string }, {
      sessionReady: true,
      activeCommandTraceId: "trace_move_selection"
    });
    (adapter as unknown as { realtimeTraceUserTranscripts: Map<string, { input: string }> }).realtimeTraceUserTranscripts.set(
      "trace_move_selection",
      { input: "把天氣小工具移到右上角" }
    );

    (adapter as unknown as { handleFunctionCall: (call: unknown) => void }).handleFunctionCall({
      id: "select_move_over_state",
      name: "assistant.select_tool",
      arguments: {
        name: "assistant.get_desktop_state",
  ...
  it("continues from direct audio candidates without waiting for transcription or injecting input_text", () => {
    const diagnostics: Array<{ type: string; status?: string; toolName?: string; data?: { selectedModule?: string } }> = [];
    ...
    Object.assign(adapter as unknown as {
      ...
      sessionReady: true,
      connectMode: "audio",
      activeCommandTraceId: "trace_early_selection",
      dataChannel: {
        readyState: "open",
        send(payload: string) {
    ...
    (adapter as unknown as { handleFunctionCall: (call: unknown) => void }).handleFunctionCall({
      id: "select_before_transcript",
      name: "assistant.select_tool",
      arguments: {
        name: "widget.remove",
        candidateTools: ["widget.remove"],
        selectedModule: "messageBoard",
        targetHint: "留言板",
        userCommand: "把留言板关掉"
      },
      source: "realtime"
    ...
    expect(diagnostics).toEqual(expect.arrayContaining([
      expect.objectContaining({
        type: "realtime.tool_selection.success",
        toolName: "widget.remove",
        data: expect.objectContaining({ selectedModule: "messageBoard" })
      })
    ]));
    expect(JSON.stringify(sent)).toContain("widget__dot__remove");
    expect(JSON.stringify(sent)).toContain("同一 conversation 中的原始客户语音");
    expect(sent).not.toEqual(expect.arrayContaining([
      expect.objectContaining({ type: "conversation.item.create" })
    ]));
    const sentBeforeTranscript = sent.length;

    (adapter as unknown as { handleRealtimeUserTranscript: (text: string, traceId: string, itemId: string) => void })
      .handleRealtimeUserTranscript("把留言板關掉", "trace_early_selection", "item_early");

    expect(sent).toHaveLength(sentBeforeTranscript);
    expect(diagnostics).not.toEqual(expect.arrayContaining([
      expect.objectContaining({ type: "realtime.tool_selection.transcript_wait" })
    ]));
  });
  ...
  it("binds a target-required window action to the only mounted widget when speech loses its name", async () => {
    const calls: Array<{ name: string; arguments: Record<string, unknown> }> = [];
    const adapter = new OpenAIRealtimeWebRtcAdapter({
      onFunctionCall(call) {
    ...
    (adapter as unknown as { handleFunctionCall: (call: unknown) => void }).handleFunctionCall({
      id: "move_only_widget_after_noisy_asr",
      name: "widget.move",
      arguments: { widgetId: "wd_hallucinated_target", x: 12, y: 24 },
      transcript: "马六甲版小东聚银在右上角",
      source: "realtime"
    });
    await Promise.resolve();
    ...
    expect(calls).toEqual([
      expect.objectContaining({
        name: "widget.move",
        arguments: expect.objectContaining({ widgetId: "wi_message", x: 12, y: 24 })
      })
    ]);
  });

  it("repairs an empty weather follow-up from the original command", async () => {
    const calls: Array<{ name: string; arguments: Record<string, unknown> }> = [];
    const adapter = new OpenAIRealtimeWebRtcAdapter({
      onFunctionCall(call) {
        calls.push(call as { name: string; arguments: Record<string, unknown> });
    ...
    (adapter as unknown as { handleFunctionCall: (call: unknown) => void }).handleFunctionCall({
      id: "weather_empty_follow_up",
      name: "board.add_widget",
      arguments: {
        definitionId: "wd_weather",
        followUp: { name: "weather.set_city", arguments: {} }
      },
      transcript: "打开天气，看看北京今天什么天气",
      source: "realtime"
    });
    await Promise.resolve();
    ...
    expect(calls).toEqual([
      expect.objectContaining({
        ...
        arguments: expect.objectContaining({
          definitionId: "wd_weather",
          followUp: { name: "weather.set_city", arguments: { city: "北京" } }
        })
      })
    ]);
  });
  ...
  it("leaves candidate-mode close-all-window selection to scoped Realtime", async () => {
    const calls: unknown[] = [];
    const sent: unknown[] = [];
    const adapter = new OpenAIRealtimeWebRtcAdapter({
      onFunctionCall: (call) => {
    ...
    expect(calls).toEqual([]);
    expect(JSON.stringify(sent)).not.toContain("local_bulk_shortcut");
    expect(JSON.stringify(sent)).toContain("widget__dot__remove");
    expect(JSON.stringify(sent)).toContain("app__dot__fullscreen__dot__set");
    expect(JSON.stringify(sent)).toContain("board__dot__add_widget");
  });
  ...
  it("leaves candidate-mode translation and missing-widget handling to scoped Realtime", async () => {
    const calls: unknown[] = [];
    const sent: unknown[] = [];
    const adapter = new OpenAIRealtimeWebRtcAdapter({
      onFunctionCall(call) {
    ...
    expect(calls).toEqual([]);
    expect(JSON.stringify(sent)).toContain("translate__dot__set_draft");
    expect(JSON.stringify(sent)).toContain("board__dot__add_widget");
  });
  ...
  it("lets low-confidence candidate lists reach the scoped Realtime decision stage", () => {
    const diagnostics: Array<{ type: string; status?: string; toolName?: string }> = [];
    const sent: unknown[] = [];
    const adapter = new OpenAIRealtimeWebRtcAdapter({
      onDiagnostic(event) {
    ...
    adapter.updateContext({
      ...
      boardName: "默认桌板",
      widgetCountsByType: { tv: 1 },
      widgets: [{ widgetId: "wi_tv", definitionId: "wd_tv", type: "tv", name: "电视", order: 1, summary: "BBC News" }]
    });
    Object.assign(adapter as unknown as { sessionReady: boolean; dataChannel: { readyState: string; send: (payload: string) => void } }, {
      sessionReady: true,
      dataChannel: {
    ...
    (adapter as unknown as { handleFunctionCall: (call: unknown) => void }).handleFunctionCall({
      id: "select_tv_candidates_zero_confidence",
      name: "assistant.select_tool",
      arguments: {
        name: "tv.play",
        candidateTools: ["tv.play", "tv.select_channel"],
        selectedModule: "tv",
        targetHint: "BBC News",
        userCommand: "播放 BBC News",
        confidence: 0
      },
      source: "realtime"
    });
    ...
    expect(diagnostics).toEqual(expect.arrayContaining([
      expect.objectContaining({ type: "realtime.tool_selection.success", status: "success", toolName: "tv.play" })
    ]));
    expect(diagnostics).not.toEqual(expect.arrayContaining([
      expect.objectContaining({ type: "realtime.tool_selection.low_confidence" })
    ]));
  ...
  it("does not override Realtime when it selects music.search", () => {
    const sent: unknown[] = [];
    const adapter = new OpenAIRealtimeWebRtcAdapter();
    adapter.updateTools([
      {
  ...
  it("keeps every Realtime TV candidate for the final audio-grounded choice", () => {
    const sent: unknown[] = [];
    const adapter = new OpenAIRealtimeWebRtcAdapter();
    adapter.updateTools([
      {
    ...
    adapter.updateContext({
      boardId: "board_1",
      boardName: "默认桌板",
      widgetCountsByType: { music: 1, tv: 1 },
      focusedWidget: {
        widgetId: "wi_music",
        definitionId: "wd_music",
        type: "music",
      ...
      widgets: [
        { widgetId: "wi_music", definitionId: "wd_music", type: "music", name: "音乐播放器", order: 1, summary: "正在播放", focused: true },
        { widgetId: "wi_tv", definitionId: "wd_tv", type: "tv", name: "电视", order: 2, summary: "CCTV1" }
      ]
    });
    Object.assign(adapter as unknown as { sessionReady: boolean; dataChannel: { readyState: string; send: (payload: string) => void } }, {
      sessionReady: true,
    ...
    (adapter as unknown as { handleFunctionCall: (call: unknown) => void }).handleFunctionCall({
      id: "select_tv_with_focused_music",
      name: "assistant.select_tool",
      arguments: {
        name: "tv.play",
        candidateTools: ["tv.play", "music.play"],
        selectedModule: "tv",
        targetHint: "CCTV1",
        userCommand: "播放 CCTV1",
        confidence: 0.95
      },
      source: "realtime"
    });

    expect(JSON.stringify(sent)).toContain("tv__dot__play");
    expect(JSON.stringify(sent)).toContain("music__dot__play");
  });

  it("trusts a registered Realtime candidate even when local text exposure has no matching phrase", () => {
    const sent: unknown[] = [];
    const diagnostics: Array<{ type: string; data?: Record<string, unknown> }> = [];
    const adapter = new OpenAIRealtimeWebRtcAdapter({
      onDiagnostic(event) {
        diagnostics.push(event as { type: string; data?: Record<string, unknown> });
      }
    });
    adapter.updateTools([
      {
    ...
    (adapter as unknown as { handleFunctionCall: (call: unknown) => void }).handleFunctionCall({
      id: "select_unseen_tv_paraphrase",
      name: "assistant.select_tool",
      arguments: {
        name: "tv.play",
        candidateTools: ["tv.play"],
        selectedModule: "tv",
        targetHint: "一号信号",
        userCommand: "把一号信号接过来",
        confidence: 0.91
      },
      source: "realtime"
    });

    const exposure = diagnostics.find((event) => event.type === "realtime.tool_exposure.plan");
    expect(exposure?.data?.semanticModules).not.toContain("tv");
    expect(JSON.stringify(sent)).toContain("tv__dot__play");
    expect(JSON.stringify(sent)).not.toContain("board__dot__add_widget");
    expect(JSON.stringify(sent)).not.toContain("music__dot__play");
  });

  it("leaves the final action to scoped Realtime when a module has multiple candidates", () => {
    const sent: unknown[] = [];
    const adapter = new OpenAIRealtimeWebRtcAdapter();
    adapter.updateTools([
      {
    ...
    (adapter as unknown as { handleFunctionCall: (call: unknown) => void }).handleFunctionCall({
      id: "select_music_ranked_candidates",
      name: "assistant.select_tool",
      arguments: {
        candidateTools: ["music.search", "music.play"],
        selectedModule: "music",
        targetHint: "王菲 红豆",
        userCommand: "播放王菲的红豆",
        confidence: 0.95
    ...
    const responseCreate = sent.find(
      (event) => (event as { type?: string }).type === "response.create"
    ) as { response?: { tool_choice?: unknown; tools?: Array<{ name?: string }>; instructions?: string } } | undefined;
    expect(responseCreate?.response?.tool_choice).toBe("required");
    expect(responseCreate?.response?.tools?.map((tool) => tool.name)).toEqual(
      expect.arrayContaining(["music__dot__search", "music__dot__play"])
    );
    expect(responseCreate?.response?.instructions).toContain("最终状态应为播放时直接调用 music.play");
    expect(responseCreate?.response?.instructions).toContain("只有最终状态仅需发现结果时才调用 music.search");
  });

  it("preserves Realtime candidate ordering when music intent is omitted", () => {
    const diagnostics: Array<{ type: string; toolName?: string; data?: unknown }> = [];
    const sent: unknown[] = [];
    const adapter = new OpenAIRealtimeWebRtcAdapter({
      onDiagnostic(event) {
  ...
  it("gives scoped Realtime both focus and add candidates for an existing music widget", () => {
    const diagnostics: Array<{ type: string; toolName?: string; data?: unknown }> = [];
    const sent: unknown[] = [];
    const adapter = new OpenAIRealtimeWebRtcAdapter({
      onDiagnostic(event) {
    ...
    (adapter as unknown as { handleFunctionCall: (call: unknown) => void }).handleFunctionCall({
      ...
      arguments: {
        name: "widget.focus",
        candidateTools: ["widget.focus", "board.add_widget"],
        selectedModule: "music",
        targetHint: "音乐播放器",
        userCommand: "打开音乐播放器",
        confidence: 0.9
    ...
    expect(diagnostics).toEqual(expect.arrayContaining([
      expect.objectContaining({
        ...
        data: expect.objectContaining({
          candidateTools: ["widget.focus", "board.add_widget"],
          scopedTools: ["widget.focus", "board.add_widget"]
        })
      })
    ]));
    expect(sent).toEqual(expect.arrayContaining([
      expect.objectContaining({
        ...
        response: expect.objectContaining({
          ...
          tools: expect.arrayContaining([
            expect.objectContaining({ name: "widget__dot__focus" }),
            expect.objectContaining({ name: "board__dot__add_widget" })
          ])
        })
      })
    ]));
  ...
  it("retries the scoped realtime tool stage once when the model finishes without a real tool call", () => {
    ...
    adapter.updateContext({
      ...
      boardName: "默认桌板",
      widgetCountsByType: { music: 1 },
      widgets: [{ widgetId: "wi_music", definitionId: "wd_music", type: "music", name: "音乐", order: 1, summary: "待命" }]
    });
    Object.assign(adapter as unknown as { sessionReady: boolean; dataChannel: { readyState: string; send: (payload: string) => void } }, {
      sessionReady: true,
      dataChannel: {
    ...
    (adapter as unknown as { functionCallTraceIds: Map<string, string> }).functionCallTraceIds.set("call_select_music", "trace_music");

    adapter.sendToolResult(
      {
        id: "call_select_music",
        name: "assistant.select_tool",
        arguments: { selectedModule: "music", targetHint: "Nick Drake Pink Moon", userCommand: "听点 Nick Drake 的 Pink Moon" },
        source: "realtime"
      },
      {
        status: "success",
        message: "候选工具已暴露",
        data: {
          userCommand: "听点 Nick Drake 的 Pink Moon",
          targetHint: "Nick Drake Pink Moon",
          scopedTools: ["music.play"]
        }
      }
    );

    expect(sent.filter((event) => (event as { type?: string }).type === "response.create")).toHaveLength(1);
    (
      adapter as unknown as {
        handleRealtimeLifecycleEvent: (event: Record<string, unknown>, commandTraceId?: string) => void;
      }
    ).handleRealtimeLifecycleEvent({ type: "response.done", response: { id: "resp_scoped_1" } }, "trace_music");
    (
      adapter as unknown as {
        handleRealtimeLifecycleEvent: (event: Record<string, unknown>, commandTraceId?: string) => void;
      }
  ...
  it("does not retry the scoped realtime tool stage after a real tool call arrives", () => {
    const sent: unknown[] = [];
    const calls: unknown[] = [];
    const adapter = new OpenAIRealtimeWebRtcAdapter({
      onFunctionCall(call) {
    ...
    Object.assign(
      adapter as unknown as {
        sessionReady: boolean;
        activeCommandTraceId: string | null;
        dataChannel: { readyState: string; send: (payload: string) => void };
      },
      {
        sessionReady: true,
        activeCommandTraceId: "trace_music",
        dataChannel: {
          readyState: "open",
          send(payload: string) {
            sent.push(JSON.parse(payload) as unknown);
    ...
    adapter.sendToolResult(
      {
        id: "call_select_music",
        name: "assistant.select_tool",
        arguments: { selectedModule: "music", targetHint: "Kraftwerk The Model", userCommand: "播放 Kraftwerk 的 The Model" },
        source: "realtime"
      },
      {
        status: "success",
        message: "候选工具已暴露",
        data: {
          userCommand: "播放 Kraftwerk 的 The Model",
          targetHint: "Kraftwerk The Model",
          scopedTools: ["music.play"]
        }
      }
    );
    (
      ...
      item: {
        type: "function_call",
        call_id: "call_music_play",
        name: "music__dot__play",
        arguments: JSON.stringify({ widgetId: "wi_music", query: "Kraftwerk The Model" })
      }
    });
    (
      adapter as unknown as {
    ...
    ).handleRealtimeLifecycleEvent({ type: "response.done", response: { id: "resp_scoped" } }, "trace_music");

    expect(calls).toEqual([
      expect.objectContaining({
        id: "call_music_play",
        name: "music.play",
        arguments: { widgetId: "wi_music", query: "Kraftwerk The Model" }
      })
    ]);
    expect(sent.filter((event) => (event as { type?: string }).type === "response.create")).toHaveLength(1);
  });
  ...
  it("lets scoped Realtime choose between a content tool and add-widget when no instance exists", () => {
    const sent: unknown[] = [];
    const adapter = new OpenAIRealtimeWebRtcAdapter();
    adapter.updateTools([
      {
    ...
    expect(sent).toEqual([
      ...
      }),
      {
        ...
        response: {
          ...
          tool_choice: "required",
          tools: expect.arrayContaining([
            expect.objectContaining({ name: "todo__dot__add_item" }),
            expect.objectContaining({ name: "board__dot__add_widget" })
          ])
        }
      }
    ...
    expect(JSON.stringify(sent[2])).toContain("todo__dot__add_item");
  });
